...
%SQLCMD% -S "%SERVER%" %AUTH% -d master -i "%SCRIPT_DIR%P07R01_BD_A_DropView.sql" -b
if errorlevel 1 goto error                        
												  
set "DB=GenJunCCR"                                   
echo Ejecutando Scrip B: Eliminando Triggers...   
%SQLCMD% -S "%SERVER%" %AUTH% -d "%DB%" -i "%SCRIPT_DIR%P07R01_BD_B_DropStoreProcedure.sql" -b
if errorlevel 1 goto error                        
												  
echo Ejecutando Scrip C: Eliminando StoredProcedures...
%SQLCMD% -S "%SERVER%" %AUTH% -d "%DB%" -i "%SCRIPT_DIR%P07R01_BD_C_TruncateTable.sql" -b
if errorlevel 1 goto error                        
...
%SQLCMD% -S "%SERVER%" %AUTH% -d "%DB%" -i "%SCRIPT_DIR%P07R01_BD_D_DropTable.sql" -b
if errorlevel 1 goto error                        
												  
echo Ejecutando Scrip F: Eliminando BaseDeDatos...
%SQLCMD% -S "%SERVER%" %AUTH% -d "%DB%" -i "%SCRIPT_DIR%P07R01_BD_E_DropDatabase.sql" -b
if errorlevel 1 goto error
